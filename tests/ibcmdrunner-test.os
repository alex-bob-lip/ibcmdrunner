...
Процедура ТестДолжен_ВыгрузитьДанныеФайловойБазыВФайл() Экспорт
    ...
    Если УправлениеИБ.СравнитьВерсии(УправлениеИБ.Версия(), "8.3.22") > 0 Тогда
		КаталогИБ = ОбъединитьПути(ТекущийСценарий().Каталог, "fixtures", "8_3_22", "db");
	ИначеЕсли УправлениеИБ.СравнитьВерсии(УправлениеИБ.Версия(), "8.3.21") > 0 Тогда
...
Процедура ТестДолжен_ВыгрузитьКонфигурациюФайловойБазыВФайлы() Экспорт
	...
	Если УправлениеИБ.СравнитьВерсии(УправлениеИБ.Версия(), "8.3.22.1703") > 0 Тогда
		КаталогИБ = ОбъединитьПути(ТекущийСценарий().Каталог, "fixtures", "8_3_22", "db");
