...
Процедура ТестДолжен_СнятьСПоддержкиКонфигурацию() Экспорт
	...
	РазмерБазыДанныхБезПоддержки = ФайлБазыДанных.Размер();
	
	Утверждения.ПроверитьИстину(РазмерБазыДанныхБезПоддержки <> РазмерБазыДанных, "Размер БД должен измениться.");
...
<<<<<<< HEAD
	...
	КонецЕсли;		
	Утверждения.ПроверитьИстину(ТестПрошел, "Строка идентификатора должна иметь корректный размер.");
=======
	
	Утверждения.ПроверитьИстину(СтрДлина(Рез) = 43, "Строка идентификатора должна иметь корректный размер.");
>>>>>>> 9f7b61937cf9c1268ae69713ba8d2bc5241f259e
