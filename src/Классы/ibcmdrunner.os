...
Процедура УстановитьПараметрыАвтономногоСервера(
    ...
    Если СравнитьВерсии(Версия, "8.3.20") > 0 Тогда
        ПараметрыАвтономногоСервера.Добавить("--data=" + КаталогДанныхАС);
    КонецЕсли;
...
//   Локаль - Строка - имя локали 
//   СмещениеДат - Число - Смещение дат в годах (имеет смысл только для MSSQL)
//      !!! Следующие параметры будут игнорироваться, если версия платформы 1C < 8.3.20 !!!
//   ИзАрхива - Булево - если Истина, то загружать из zip-архива. Параметр ПутьККаталогуВыгрузки должен 
//      быть именем архива     
Процедура СоздатьИБИзФайловКонфигурации(
    Знач ПутьККаталогуВыгрузки,
    Знач Локаль = Неопределено,
    Знач СмещениеДат = 2000,
    Знач ИзАрхива = Ложь) Экспорт
...
// Параметры:
//   ИмяРасширения - Строка - Имя расширения
//   ДинамическоеОбновление - Строка - Использование динамического обновления.
...
//      эксклюзивной блокировки информационной базы.
//      auto - автоматически (по-умолчанию)
//      disable - использование запрещено
//      prompt - запрос пользователя
...
Процедура ОбновитьКонфигурациюБазыДанных(
    ...
    ПараметрыЗапуска.Добавить("--force");
    Если СравнитьВерсии(Версия(), "8.3.20") Тогда
        Ожидаем.Что(СтрНайти(ДинамическоеОбновление, "auto,disable,prompt,force") > 0, 
            "Должно быть одно из auto, disable, prompt, force").ЭтоИстина();
        Ожидаем.Что(СтрНайти(ЗавершатьСеансы, "auto,disable,prompt,force") > 0, 
            "Должно быть одно из auto, disable, prompt, force").ЭтоИстина(); 
...
Процедура ВыгрузитьКонфигурациюВФайлы(
    Знач КаталогВыгрузки,
    Знач ПутьКФайлуXMLВыгрузки = Неопределено,
    Знач ИмяРасширения = "",
...
Процедура ДобавитьОбщиеПараметрыИБ()
    ...
    Если ПараметрыСервернойИБ.Количество() > 0 Тогда
        ...
        КонецЦикла;СлужебныеПроцедурыИФункции
